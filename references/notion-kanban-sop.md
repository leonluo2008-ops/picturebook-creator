...
**状态机（派生视图，不手拉）**：待产(素材库原始行) →(Agent push 预处理产物,自动置)→ **待审核** →(用户点选定标题/选定简介+设排产时间,页面自动化)→ 已排产 →(Agent勾领取,自动化+poll兜底)→ 已领取（生产中）→(Agent交付)→ 已交付；弃用旁路。

**工段边界 = 状态边界**：素材预处理工段（L1-L3）的完成标志就是状态翻「待审核」——你在台账里一眼扫出哪些册 Agent 已备好料等你审（紫标），哪些还没做（灰标）。人工审核动作 = 点选定标题/选定简介 + 设排产时间，不拉状态。
...
| 转移 | 触发者 | 触发动作 |
|---|---|---|
| 待产→待审核 | Agent | push（素材预处理完成的标志） |
| 待审核→已排产 | 用户（点选定+设排产时间） | Notion 自动化规则1（条件=待审核） |
| 已排产→已领取（生产中） | Agent | poll 领取（勾选+置状态，原子） |
...
数据库 ⚡ → New automation：
1. 触发「排产时间 edited」+条件 状态=待审核 → 动作设状态=已排产
2. 触发「Agent已领取 checked」→ 动作设状态=已领取（生产中）

（自动化规则只能页面建，API 建不了；poll 自带兜底：到期但状态仍待审核/待产会提示。**注**：已有自动化条件若是「状态=待产」的旧规则，改为「状态=待审核」。）
