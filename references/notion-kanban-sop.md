...
**状态机（派生视图，不手拉）**：待产(素材库原始行) →(Agent push 预处理产物,自动置)→ **待审核** →(用户审核裁决,**手动置**)→ **已审核** →(页面自动化规则1)→ 已排产 →(Agent勾领取,自动化+poll兜底)→ 已领取（生产中）→(Agent交付)→ 已交付；弃用旁路。
...
（自动化规则只能页面建，API 建不了；poll 自带兜底：待审核+到期、已审核滞留、待产+到期都会分别提示根因。）
